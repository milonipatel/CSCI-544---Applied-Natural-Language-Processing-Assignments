...
USC_EMAIL = 'milonipa@usc.edu'  # TODO(student): Fill to compete on rankings.
PASSWORD = 'be53d628ce8e90c2'  # TODO(student): You will be given a password via email.
...
class DatasetReader(object):
    ...
    def ReadFile(filename, term_index, tag_index):
        ...
        finput = codecs.open(filename, 'r', encoding="utf-8")
        ...
        for line in lines:
            ...
            for data in value:
                ...
                parsedTerms=(term_index[word],tag_index[tag])
                if parsedLine not in parsedTerms:
                    parsedLine.append(parsedTerms)
    ...
    def BuildMatrices(dataset):
        ...
        """
        N=len(dataset)
        T=max(len(x) for x in dataset)

        a = numpy.zeros(shape=(N,T), dtype=int)
        b = numpy.zeros(shape=(N,T), dtype=int)
        c = numpy.zeros(shape=(N), dtype=int)
        j=0
        for _list in dataset:
            m=numpy.zeros(shape=(T),dtype=int)
            n=numpy.zeros(shape=(T),dtype=int)
            i=0
            for _tuple in _list:
                wordid=_tuple[0]
                m[i]=wordid
...
class SequenceModel(object):
    ...
    def __init__(self, max_length=310, num_terms=1000, num_tags=40):
        ...
        #self.tensor1 = tf.placeholder(tf.float32, [None], 'tensor1')
    # TODO(student): You must implement this.
    def lengths_vector_to_binary_matrix(self, length_vector):
        ...
        """
        #print(length_vector)
        tensor_2 = tf.sequence_mask([length_vector],self.max_length)
        print("-----")
        print(tensor_2)
        #self.tensor1=tensor_2
        return tensor_2
    ...
    def build_inference(self):
        ...
        #tf.get_variable("EMBEDDING_MATRIX", shape=[vocab,40])
        ids = tf.placeholder(tf.int64, [32, self.max_length])
        params=tf.get_variable('embeddings',[self.num_terms,40])
        xemb = tf.nn.embedding_lookup(params,ids)
        state_size=self.num_tags
        #state_size=self.max_length
    ...
    def build_training(self):
        ...
        Hint:
            ...
            - tf.losses.get_total_loss() should return a valid tensor (without raising
                ...
                non-empty list.
        """
        self.tags1 = tf.placeholder(tf.int64, [None, self.max_length], 'tags1')
        #target_weights = tf.constant(numpy.ones((32, 310)), tf.float32)
        len_tensor=self.lengths_vector_to_binary_matrix(self.lengths)
        self.loss=tf.contrib.seq2seq.sequence_loss(self.logits,self.tags1,len_tensor)
        opt = tf.train.AdamOptimizer()
        self.train_op = opt.minimize(self.loss)
        #tf.train.AdamOptimizer(m)
        run_inference(self.x,self.lengths)
        pass
    ...
    def train_epoch(self, terms, tags, lengths, batch_size=32, learn_rate=1e-7):
        ...
        learning_rate=tf.placeholder_with_default(numpy.array(0.01,dtype='float32'),shape=[],name='learn_rate')
        ...
        for si in range(0,terms.shape[0],batch_size):
            ...
            sess.run(self.logits,{self.x:slice_x,self.tags1:slice_y,is_training:True,learning_rate:0.05})
        return True    
        pass
...
def main():
    ...
    model.lengths_vector_to_binary_matrix(train_lengths)
